...
#define NUIPShiftReduceGotoTableTableKey @"t"
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (nil != self)
    {
        NSArray *rows = [aDecoder decodeObjectForKey:NUIPShiftReduceGotoTableTableKey];
        capacity = [rows count];
        table = malloc(capacity * sizeof(NSMutableDictionary *));
...
{
    [aCoder encodeObject:[NSArray arrayWithObjects:table count:capacity] forKey:NUIPShiftReduceGotoTableTableKey];
}
...
{
    return [(NSNumber *)[table[state] objectForKey:[rule ruleName]] unsignedIntegerValue];
}
